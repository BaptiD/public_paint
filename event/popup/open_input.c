    ...
    sfImage *board, action_t *cursor)
{
    ...
    sfRenderWindow_drawSprite(window, sprites[OPEN_INPUT].sprite, NULL);
...
{
    int resx = pos.x - POSITION[OPEN_INPUT].x;
    int resy = pos.y - POSITION[OPEN_INPUT].y;

    if (resx < RESIZE[OPEN_INPUT].x && resx > 0)
        if (resy < RESIZE[OPEN_INPUT].y && resy > 0)
            return 1;
    return 0;
}
    ...
    action_t *cursor, sfImage *board)
{
    ...
    while (run == 1 && sfRenderWindow_isOpen(window)) {
        ...
        display_folder_files(window, text, folder_files);
        sfText_setString(text, filename);
        sfRenderWindow_drawText(window, text, NULL);
        sfRenderWindow_display(window);
    }
    sfRenderWindow_setMouseCursorVisible(window, sfFalse);
    free_arr(folder_files);
